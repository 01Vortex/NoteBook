...
│           │   └── media/
│           │       └── Media.java   # 媒体实体
│           ├── mapper(repository)/          # 可选,只有接口!sql不复杂可忽略此包
│           │   ├── UserMapper.java    
│           │   └── MediaMapper.java   
│           │── gateway/               # 基础设施模块的业务接口,尽量不要出现具体品牌的名称
│           │   ├── StorageGateway.java 
│           │   └── TokenGateway.java
│           ├── query/                 # 查询参数对象（如用于动态查询条件封装）
│           └── event/                 
│               └── BadgeUnlockEvent.java  # 徽章解锁事件
│
├── my-project-infrastructure/         # 基础设施模块放真正全局共享的技术组件
│   ├── pom.xml
│   ├── src/main/java/com/example/infrastructure/ 基础设施模块(放第三方服务)
│   │   ├── config/                    # 第三方组件自动配置类
│   │   │   ├── RedisConfig.java       # Redis 配置
│   │   │   ├── MyBatisConfig.java     # MyBatis 配置（若需要）
│   │   │   └── MinioConfig.java
│   │   ├── storage/                   # 存储服务
│   │   │   └── MinioGatewayImpl.java  # MinIO 存储服务实现
│   │   ├── auth/                      # 认证服务
│   │   │   └── JwtGatewayImpl.java    # 认证服务实现 
│   │   ├── mapper(repository)/        # 数据访问层实现(MyBatis plus)
│   │   │   ├── user/
│   │   │   │   └── UserMapper.java
│   │   │   └── media/
│   │   │       └── MediaMapper.java
